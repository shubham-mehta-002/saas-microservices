...
export const verifyRegisterOtpApi = async(data : verifyOtpType) => {
    const res = await axiosInstance.post<ApiResponse<UserModelType>>('/auth/verify-otp', data)
    return res.data;
}


export const loginUserApi = async(data: loginUserType)  =>{
    const res = await axiosInstance.post<ApiResponse<UserModelType>>('/auth/login',data)
    return res.data;
}
